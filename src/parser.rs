...
fn parse_recursive(
    ...
    file_id: usize,
) -> Result<Vec<Inst>, Diagnostic<usize>> {
    fn diag_unwarp<T>(
        option: Option<T>,
        file_id: usize,
        span: Range<usize>,
    ...
    while let Some(token) = tokens.peek() {
        match token.kind.clone() {
                ...
                let eq_token = match diag_unwarp(tokens.next(), file_id, var.span.clone())? {
                    token if token.kind != TokenKind::Eq => {
                        return Err(Diagnostic::error()
                            ...
                            .with_labels(vec![
                                Label::primary(file_id, token.span)
                                    .with_message("invalid token here"),
                                Label::secondary(file_id, var.span)
                    ...
                    match diag_unwarp(tokens.next(), file_id, var.span.start..eq_token.span.end)? {
                        ...
                        t => {
                            return Err(Diagnostic::error()
                                ...
                                .with_labels(vec![
                                    Label::primary(file_id, t.span)
                                        .with_message("invalid token here"),
                                    Label::secondary(file_id, eq_token.span)
                        ...
                        let (right, right_span) =
                            match diag_unwarp(tokens.next(), file_id, var.span.start..op.span.end)?
                            {
                                ...
                                t => {
                                    return Err(Diagnostic::error()
                                        .with_message(
                                            "Numeric operator must be followed by a variable",
                                        )
                                        .with_labels(vec![
                                            Label::primary(file_id, t.span)
                                                .with_message("invalid token here"),
                                            Label::secondary(file_id, eq_token.span)
                                                .with_message("assignment operator here"),
                                            Label::secondary(file_id, op.span)
                        ...
                        t => {
                            return Err(Diagnostic::error()
                                ...
                                .with_labels(vec![
                                    ...
                                    Label::secondary(file_id, eq_token.span)
                                        .with_message("assignment operator here"),
                                    Label::secondary(file_id, left_span)
                                        .with_message("function name here"),
                                ]));
                        }
                    };
                        ...
                        t => {
                            return Err(Diagnostic::error()
                                ...
                                .with_labels(vec![
                                    Label::primary(file_id, t.span)
                                        .with_message("invalid token here"),
                                    Label::secondary(file_id, eq_span)
                                        .with_message("assignment operator here"),
                                ]));
                        }
                    };
            ...
            TokenKind::LeftBracket => {
                ...
                if close.as_ref().map(|t| &t.kind) != Some(&TokenKind::RightBracket) {
                    return Err(Diagnostic::error()
                        ...
                        .with_labels(vec![
                            Label::primary(file_id, open.span)
                                .with_message("code block starts here"),
            ...
            TokenKind::While => {
                ...
                let (cond, cond_span) =
                    match diag_unwarp(tokens.next(), file_id, while_span.clone())? {
                        Token {
                            kind: TokenKind::Var(var),
                            span,
                        } => (var, span),
            ...
            TokenKind::For => {
                ...
                let (num, num_span) = match diag_unwarp(tokens.next(), file_id, for_span.clone())? {
                    ...
                    t => {
                        return Err(Diagnostic::error()
                            ...
                            .with_labels(vec![
                                Label::primary(file_id, t.span).with_message("invalid token here"),
                                Label::secondary(file_id, for_span.clone())
                                    .with_message("for keyword here"),
                            ]));
                    }
                };
                ...
                if do_token.kind != TokenKind::Do {
                    return Err(Diagnostic::error()
                        ...
                        .with_labels(vec![
                            Label::primary(file_id, do_token.span)
                                .with_message("invalid token here"),
                            Label::secondary(file_id, for_span)
                ...
                let od_token =
                    diag_unwarp(tokens.next(), file_id, for_span.start..do_token.span.end)?;
                if od_token.kind != TokenKind::Od {
                    return Err(Diagnostic::error()
                        .with_message("Wrong or missing code block delimiter")
                        .with_labels(vec![
                            Label::primary(file_id, od_token.span)
                                .with_message("invalid token here"),
                            Label::secondary(file_id, do_token.span)
                                .with_message("code block starts here"),
                        ]));
                }
            ...
            TokenKind::Fn => {
                ...
                let (target, target_span) =
                    match diag_unwarp(tokens.next(), file_id, fn_span.clone())? {
                        Token {
                            kind: TokenKind::Var(var),
                            span,
                        } => (var, span),
